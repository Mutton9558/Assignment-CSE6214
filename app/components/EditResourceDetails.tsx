...
export function EditResourceDetails({resourceId}: ResourceDetailsProp){
    ...
    const [roomName, setRoomName] = useState("CNMX1001");
    type ResourceTuple = [string, number];
    const [counts, setCounts] = useState<ResourceTuple[]>([["Air Conditioner",1], ["AV System", 1]]);
    ...
    return(
        <div className="bg-background max-w-screen min-h-screen m-0 p-0 box-border flex flex-col items-center overflow-x-hidden">
            <BackButton buttonName="Back" buttonDesc="Editing CNMX1001"></BackButton>
            
            <div id="resource-img-container" className="group relative w-7/8 h-72 rounded-2xl bg-gray-400 mt-24 overflow-hidden cursor-pointer">
                <div className="absolute inset-0 w-full h-full bg-gray-500/50 z-10 justify-center items-center hidden group-hover:flex">
                    <p className="text-xl font-mono font-bold text-shadow-2xs text-shadow-white">+ Edit Resource</p>
                </div>
                <Image src="/metalpipe.svg" alt="placeholder-img" fill className="object-cover z-1" sizes="33vw" loading="eager"></Image>
            </div>
            ...
            <form className="w-full">
                <div className="w-full flex flex-col relative p-8 justify-start">
                    <h1>Room Name:</h1>
                    <input id="room-name" value={roomName} onChange={(e) => setRoomName(e.target.value)} className="w-11/12 h-8 bg-accent rounded-full p-2"></input>
                </div>
                <div id="equipment-details-section" className="mb-16 w-full max-h-screen font-bold">
                    <div className="relative flex flex-row items-center mb-8 ml-[6.25%]">
                        ...
                        <button type="button" className="absolute right-0 text-sm mr-[6.25%] bg-accent p-1.5 rounded-full w-32">+ Add Resources</button>
                    </div>
                    {
                        counts.map(([name, count], index) => (
                            <div key={name} className="relative flex flex-row items-center mb-8">
                                <p className="text-md font-medium ml-[6.25%]">{name}</p>
